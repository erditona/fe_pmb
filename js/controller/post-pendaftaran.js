...
async function generateKodePendaftaran() {
  ...
  var kodePendaftaran = parseInt(randomNum);
  ...
  if (isTerdaftar) {
    ...
    kodePendaftaran = parseInt(randomNum);
  }
  ...
  Promise.all([getSchoolData(asalSekolahValue), getMajorData(jurusanValue)])
    .then(([schoolData, majorData]) => {
      ...
      generateKodePendaftaran()
        .then((kdpendaftaranValue) => {
          setValue("kdpendaftaran", kdpendaftaranValue); // Set the generated kode pendaftaran
        ...
        .catch((error) => {
          console.error(error);
          document.getElementById("status").textContent = "Failed to generate kode pendaftaran.";
        });
    })
    .catch((error) => {
      console.error(error);
